    ...
    class VisioManipulator
    {
        ...
        public void openDocument()
        {
            ...
            this.visioStencil = visioApp.Documents.OpenEx(
               Configuration.shapesMastersFilePath,
        ...
        {
            string diagramFilename = saveDiagramDocument(diagramFilePath);
            visioStencil.Close();
            visioApp.ActiveDocument.Close();
            visioApp.Quit();
        ...
        public ShapeWrapper dropShape(Node node, double x, double y)
        {
            ...
            if (node != null)
            {
                Master shapeMaster = getShapeMasterByShapeType(node.shapeForm);
                Shape shapeToDrop = visioPage.Drop(shapeMaster, x, y);
                ShapeForm newShapeType = node.shapeForm;
            ...
            {
                throw new Exception("Node to place shape can't be null!");
            }
